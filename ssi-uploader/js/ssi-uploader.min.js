(function(root, factory) {
    ...
    Ssi_upload.prototype.init = function(element) {
        $(element).addClass('ssi-uploadInput').after(this.$element = $('<div class="ssi-uploader">'));
        var $chooseBtn = $('' + '<span class="ssi-InputLabel">' + '<button class="ssi-button success">' + this.language.chooseFiles + '</button>' + '</span>').append(element);
        var $uploadBtn = $('<button id="ssi-uploadBtn" style="display:none !important;" class="ssi-button success ssi-hidden" >' + '' + '<div id="ssi-up_loading" class="ssi-btnIn"></div></button>');
        var $clearBtn = $('<button id="ssi-clearBtn" class="ssi-hidden ssi-button info" >' + this.language.clear + '</button>');
        ...
        if (!this.options.preview) {
            ...
            var $mainBox = $('<div id="ssi-uploadFiles" class="ssi-tooltip ssi-uploadFiles ' + (this.options.dropZone ? 'ssi-dropZone' : '') + '"><div id="ssi-uploadProgressNoPreview" class="ssi-uploadProgressNoPreview"></div></div>').append($namePreview);
        ...
        $uploadBox.on('click', '.ssi-removeBtn', function(e) {
            e.preventDefault();
            var $currentTarget = $(e.currentTarget);
            var index = $currentTarget.data('delete');
            thisS.pending--;
            thisS.currentListLength--;
            if (thisS.pending < 1) {
            ...
            if (thisS.options.preview) {
                $currentTarget.parents('table.ssi-imgToUploadTable').remove()
            } else {
                var target = $currentTarget.parents('tr.ssi-toUploadTr');
                $namePreview.html((thisS.currentListLength) + ' files');
                target.prev().remove();
                target.remove();
                if (thisS.currentListLength === 1) {
                    setLastElementName(thisS)
                }
            }
            thisS.toUpload[index] = null;
            thisS.imgNames[index] = null;
            if (thisS.currentListLength === 0) {
                if (!thisS.options.dropZone) {
        ...
        $uploadBox.on('click', '.ssi-abortUpload', function(e) {
            e.preventDefault();
            var $eventTarget = $(e.currentTarget);
            var index = $eventTarget.data('delete');
            thisS.abort(index)
        });
    ...
    function tooltip($target, text, thisS) {
        ...
        if (!text)
            return;
        var $toolTip = $('<div class="ssi-fadeOut ssi-fade ssi-tooltipText">' + text + '</div>').appendTo(thisS.$element);
    ...
    Ssi_upload.prototype.abortAll = function() {
        for (var i = 0; i < this.uploadList.length; i++) {
            if (typeof this.uploadList[i] === 'object') {
                this.abort(i)
            }
        }
    }
    ...
    Ssi_upload.prototype.toUploadFiles = function(files) {
        if (typeof this.options.maxNumberOfFiles === 'number') {
            if ((this.inProgress + this.pending) >= this.options.maxNumberOfFiles) {
                return
            }
        }
        var thisS = this, j = 0, length, imgContent = '', $uploadBtn = this.$element.find('#ssi-uploadBtn'), $clearBtn = this.$element.find('#ssi-clearBtn'), $fileList = this.$element.find('#ssi-fileList'), $uploadBox = this.$element.find('.ssi-uploadBox'), imgs = [];
        if ((this.inProgress === 0 && this.pending === 0)) {
            this.clear()
        }
        var extErrors = []
          , sizeErrors = []
        ...
        if (typeof this.options.maxNumberOfFiles === 'number') {
            if (filesLength > this.options.maxNumberOfFiles - (this.inProgress + this.pending)) {
                filesLength = toUploadLength = this.options.maxNumberOfFiles - (this.inProgress + this.pending)
            }
        }
        for (var i = 0; i < filesLength; i++) {
            var file = files[i]
              , ext = file.name.getExtension();
            if ($.inArray(ext, this.options.allowed) === -1) {
                if (length > filesLength) {
                    filesLength++
                } else {
                    toUploadLength--
                }
                if ($.inArray(ext, extErrors) === -1) {
                    extErrors.push(ext)
                }
            } else if ((file.size * Math.pow(10, -6)).toFixed(2) > this.options.maxFileSize) {
                if (length > filesLength) {
                    filesLength++
                } else {
                    toUploadLength--
                }
                sizeErrors.push(cutFileName(file.name, ext, 15))
            } else if ($.inArray(file.name, this.imgNames) === -1) {
        ...
        if (extErrorsLength + sizeErrorsLength > 0) {
        ...
        function setupReader() {
            ...
            $clearBtn.prop('disabled', !0);
            thisS.toUpload[index] = file;
            var filename = file.name;
            var ext = filename.getExtension();
            thisS.imgNames[index] = filename;
            if (thisS.options.preview) {
                var getTemplate = function(content) {
                    return '<table class="ssi-imgToUploadTable ssi-pending">' + '<tr><td class="ssi-upImgTd">' + content + '</td></tr>' + '<tr><td><div id="ssi-uploadProgress' + index + '" class="ssi-hidden ssi-uploadProgress"></div></td></tr>' + '<tr><td><button data-delete="' + index + '" class=" ssi-button error ssi-removeBtn"><span class="trash10 trash"></span></button></td></tr>' + '<tr><td>' + cutFileName(filename, ext, 15) + '</td></tr></table>'
                };
                ...
                if (fileType[0] == 'image') {
                    ...
                    fileReader.onload = function() {
                        imgContent += getTemplate('<img class="ssi-imgToUpload" src=""/><i class="fa-spin fa fa-spinner fa-pulse"></i>');
                ...
                thisS.$element.find('.ssi-namePreview').html((index === 0 ? cutFileName(filename, ext, 13) : (thisS.currentListLength + 1) + ' ' + thisS.language.files));
                $fileList.append('<tr class="ssi-space"><td></td></tr>' + '<tr class="ssi-toUploadTr ssi-pending"><td><div id="ssi-uploadProgress' + index + '" class="ssi-hidden ssi-uploadProgress ssi-uploadProgressNoPre"></div>' + '<span>' + cutFileName(filename, ext, 20) + '</span></td>' + '<td><a data-delete="' + index + '" class="ssi-button ssi-removeBtn  ssi-removeBtnNP"><span class="trash7 trash"></span></a></td></tr>')
            }
            var setImg = function() {
                for (var i = 0; i < imgs.length; i++) {
                    if (imgs[i] !== null) {
                        $uploadBox.find("#ssi-uploadProgress" + i).parents('table.ssi-imgToUploadTable').find('.ssi-imgToUpload').attr('src', imgs[i]).next().remove();
                        imgs[i] = null
                    }
                }
                imgs = []
    ...
    var clearPending = function(thisS) {
        ...
        thisS.pending = 0;
        if (!thisS.options.preview)
            $pending.prev('tr').remove();
        $pending.remove()
    };
    Ssi_upload.prototype.clear = function(action) {
        switch (action) {
        case 'pending':
            clearPending(this);
            break;
    ...
    ;
    var setNamePreview = function(thisS) {
        if (thisS.currentListLength > 1) {
            thisS.$element.find('.ssi-namePreview').html(thisS.currentListLength + ' files')
        } else if (thisS.currentListLength === 1) {
            setLastElementName(thisS)
        } else {
            thisS.$element.find('.ssi-uploadDetails').removeClass('ssi-uploadBoxOpened');
            thisS.$element.find('#ssi-fileList').empty();
            thisS.$element.find('.ssi-namePreview').empty()
        }
    };
    Ssi_upload.prototype.uploadFiles = function() {
        if (this.pending > 0) {
            ...
            this.pending = 0;
            if (this.inProgress === this.currentListLength) {
                $clearBtn.prop("disabled", !0)
            }
            while (thisS.toUpload[i] === null || thisS.toUpload[i] === '') {
                i++
            }
        ...
        function ajaxLoopRequest(formData, ii) {
            ...
            var uploadBar = thisS.$element.find('#ssi-uploadProgress' + ii);
            uploadBar.removeClass('ssi-hidden').parents(selector).removeClass('ssi-pending');
            var ajaxOptions = $.extend({}, {
                xhr: function() {
                    ...
                    xhr.upload.addEventListener('progress', function(e) {
                        if (e.lengthComputable) {
                            ...
                            thisS.totalProgress[ii] = percentComplete;
                            var sum = arraySum(thisS.totalProgress) / (thisS.inProgress + thisS.successfulUpload);
                            if (!thisS.options.preview) {
                                thisS.$element.find('#ssi-uploadProgressNoPreview').removeClass('ssi-hidden').css({
                                    width: sum + '%'
                                })
                            }
                            $uploadBtn.find('#ssi-up_loading').html(Math.ceil(sum) + '%')
                        }
                    }, !1);
                    return xhr
                },
                ...
                beforeSend: function(xhr, settings) {
                    ...
                    $uploadBtn.find('#ssi-up_loading').html('<i class="fa fa-spinner fa-pulse"></i>');
                    if (typeof thisS.options.beforeEachUpload === 'function') {
                        try {
                            var msg = thisS.options.beforeEachUpload({
                            ...
                            if (err.name == 'Error') {
                                thisS.abort(ii, undefined, err.message)
                            } else {
                                if (!thisS.options.ignoreCallbackErrors) {
                                    ...
                                    console.log(err);
                                    thisS.abort(ii, undefined, thisS.language.wentWrong)
                                }
                            }
                            return
                        }
                ...
                error: function(request, error) {
                    if (error !== 'abort') {
                        ...
                        thisS.abortedWithError++;
                        thisS.totalProgress.splice(ii, 1);
                        if (!thisS.options.preview) {
                        ...
                        thisS.inProgress--;
                        $clearBtn.prop("disabled", !1);
                        if (typeof thisS.options.onEachUpload === 'function') {
                            try {
                                thisS.options.onEachUpload({
                                    uploadStatus: 'error',
                ...
                if (thisS.options.responseValidation) {
                    ...
                    if (typeof valData.validationKey === 'object' && valData.resultKey == 'validationKey') {
                        if (data.hasOwnProperty(valData.validationKey.success)) {
                            cb(!0, data[valData.validationKey.success])
                        } else {
                            cb(!1, data[valData.validationKey.error])
                        }
                    } else {
                        if (data[valData.validationKey] == valData.success) {
                            cb(!0, data[valData.resultKey])
                        } else {
                            cb(!1, data[valData.resultKey])
                        }
                    }
                } else {
                    if (jqXHR.status == 200) {
    ...
    var setElementMessage = function(thisS, index, msgType, msg, title) {
        ...
        element.after(getResultMessage(msgType, msg, title, className)).remove()
    };
    var getCompleteStatus = function(thisS) {
        return (thisS.inProgress === 0)
    };
    var getResultMessage = function(type, msg, title, classes) {
        var callback_msg = title;
        var success_label_type = 'success';
        var error_label_type = 'error';
        var error_msg = 'File Upload Error';
        if (title == "") {
            localStorage.setItem('callback', title);
            return '<span class="ssi-statusLabel ' + classes + ' ' + success_label_type + '" data-status="' + title + '">' + msg + '</span>'
        } else {
            localStorage.setItem('callback', title);
            return '<span class="ssi-statusLabel ' + classes + ' ' + error_label_type + '" data-status="' + title + '">' + error_msg + '</span>'
        }
    };
    var getCurrentListLength = function(thisS) {
        return (thisS.inProgress + thisS.successfulUpload + thisS.aborted + thisS.abortedWithError + thisS.pending)
    };
    var setLastElementName = function(thisS) {
        var fileName = thisS.$element.find('#ssi-fileList').find('span').html();
        var ext = fileName.getExtension();
        thisS.$element.find('.ssi-uploadDetails').removeClass('ssi-uploadBoxOpened');
        thisS.$element.find('.ssi-namePreview').html(cutFileName(fileName, ext, 15))
    };
    Ssi_upload.prototype.abort = function(index, title, mmsg) {
        ...
        this.inProgress--;
        if (getCompleteStatus(this)) {
            finishUpload(this)
        }
    }
    ;
    var finishUpload = function(thisS) {
        thisS.$element.find('#ssi-abortBtn').addClass('ssi-hidden');
        if (!thisS.options.preview) {
            var type = 'error'
              , title = ''
              , msg = '';
            if (thisS.abortedWithError > 0) {
                if (thisS.totalFilesLength === 1) {
                    title = thisS.errors
                } else {
                    title = thisS.language.someErrorsOccurred
                }
                msg = '<span class="exclamation23"></span>'
            } else if (thisS.aborted > 0 && thisS.successfulUpload === 0) {
                msg = '<span class="ban23"></span>';
                title = thisS.language.aborted
            } else if (thisS.successfulUpload > 0) {
                type = 'success';
                msg = '<span class="check23"></span>';
                title = thisS.language.sucUpload
            }
            thisS.$element.find('.ssi-namePreview').append(getResultMessage(type, msg, title, 'ssi-noPreviewMessage'));
            thisS.$element.find('#ssi-uploadProgressNoPreview').removeAttr('styles').addClass('ssi-hidden')
        }
        if (typeof thisS.options.onUpload === 'function') {
            try {
                thisS.options.onUpload()
            } catch (err) {
                if (!thisS.options.ignoreCallbackErrors) {
        ...
        thisS.$element.find('#ssi-clearBtn').prop("disabled", !1);
        $uploadBtn.prop("disabled", !1).find('#ssi-up_loading').empty();
        if (thisS.pending === 0) {
            $uploadBtn.addClass('ssi-hidden');
            thisS.toUpload = [];
            thisS.imgNames = [];
    ...
    $.fn.ssi_uploader = function(opts) {
        ...
        var options = $.extend(!0, defaults, opts);
        options.allowed = options.allowed || ['jpg', 'jpeg', 'png'];
        return this.each(function() {
            var $element = $(this);
            if ($element.is('input[type="file"]')) {
                if ($element.data('ssi_upload'))
                    return;
    ...
    ;
    var cutFileName = function(word, ext, maxLength) {
