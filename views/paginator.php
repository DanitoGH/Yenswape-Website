...
class Paginator {
    ...
    public function getData($limit = 20, $page = 1,$q) { //set default argument values
    ...
    public function createLinks($links,$list_class)
    {
        ...
        '<li class="' . $class . '"><a href="?'.$this->que.'&limit=' . $this->_limit . '&page=' . ( $this->_page - 1 ) . '">&laquo;</a></li>';

        $html .= $previous_page;

        if ( $start > 1 ) { //print ... before (previous <<< link)
            $html .= '<li><a href="?'.$this->que.'&limit=' . $this->_limit . '&page=1">1</a></li>'; //print first page link
        ...
        for ( $i = $start ; $i <= $end; $i++ ) {
            ...
            $html .= '<li class="' . $class . '"><a href="?'.$this->que.'&limit=' . $this->_limit . '&page=' . $i . '">' . $i . '</a></li>';
        }

        if ( $end < $last ) { //print ... before next page (>>> link)
            $html .= '<li class="disabled"><span>...</span></li>'; //print 3 dots if not on last page
            $html .= '<li><a href="?'.$this->que.'&limit=' . $this->_limit . '&page=' . $last . '">' . $last . '</a></li>'; //print last page link
        }
        ...
        '<li class="' . $class . '"><a href="?'.$this->que.'&limit=' . $this->_limit . '&page=' . ( $this->_page + 1 ) . '">&raquo;</a></li>';
    ...
    public function counter($links,$list_class)
    {
        //return empty result string, no links necessary
        if ( $this->_limit == 'all' ) {
            return '';
        ...
        echo $this->_page.'&nbsp;-&nbsp'.$last.'&nbsp; of &nbsp;'.$this->_total.'&nbsp;ads';
